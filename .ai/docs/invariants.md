...
| 29 | A partial commit reads as verified, because the digest survives commits | No | The tree digest keys on CONTENT so that a walk survives the commit it made safe — which means it cannot say *which* code was committed. Commit half a verified change and the digest is honestly fresh (the code on disk is what ran) while `HEAD` carries something no walk saw, and a gate reading exit 0 ships exactly that. `pipeline:verify` refuses one combination and only one: the commit moved since the run AND the tree still holds uncommitted changes. Clean-with-a-moved-commit is the transition the digest exists to allow; dirty-with-an-unchanged-commit is ordinary mid-work verification. Unknown never refuses, on either half, because a digest from another algorithm says nothing about what is shipping |
| 23 | `pipeline:history` or `pipeline:list` exit 0 is read as a pass | No | Both report rather than gate, and exit 0 for every answer they can give — an empty history, a stale run, a failed run, a pipeline declaring no steps — so nobody can read a zero as a verdict. Non-zero means only that the command could not answer, and `pipeline:list` takes no arguments so it has no such case. `pipeline:verify` owns the gate exit code, and the README says so beside all three commands |
