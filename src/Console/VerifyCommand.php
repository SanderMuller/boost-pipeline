...
final class VerifyCommand extends Command
{
    ...
    public function handle(Pipelines $pipelines, ReceiptStoreFactory $receipts, TreeFingerprint $tree): int
    {
        ...
        // Before the verdict: a walk that dropped a gate, or that a mistyped tag
        // narrowed to the untagged steps, can hold nothing but passes, and the
        // verdict guard would then name no step at all.
    ...
    private function answerServerVerified(Pipelines $pipelines, string $name, Receipt $receipt, ?string $now): int
    {
        ...
        //    A recorded `incomplete` was refused before this method. Absent means
        //    the receipt predates the field, which the bare call tolerates and
        //    this flag does not: unknown is not clean.
     ...
     *
     * Only a recorded `incomplete` answers here. Absent is unknown, which the bare
     * call tolerates as it tolerates an absent config digest; `--server-verified`
     * refuses it itself.
     *
     * The receipt records THAT coverage broke, never which notice broke it, so the
     * message can only be specific where the config on disk can be asked. A scoped
     * receipt whose tag no declared step carries is the one case it can settle.
     * Everything else gets the generic sentence, which names both causes.
     */
